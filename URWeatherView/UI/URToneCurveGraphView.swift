...
public let DefaultToneCurveInputs: [CGPoint] = [.zero, CGPoint(x: 0.25, y: 0.25), CGPoint(x: 0.5, y: 0.5), CGPoint(x: 0.75, y: 0.75), CGPoint(x: 1.0, y: 1.0)]
...
//        self.line.frame = CGRect(x: 0, y: 0, width: self.bounds.width, height: self.bounds.height)
        ...
        if gesture.state == .ended {
            ...
            if self.curveVectorDots.count > 0 {
                ...
                for (index, dot) in self.curveVectorDots.enumerated() {
                    ...
                    guard let _ = dot.hitTest(position, with: nil) else {
    ...
    @objc func handlePan(_ gesture: UIPanGestureRecognizer) {
        ...
        case .changed:
            ...
            guard let _ = self.hitTest(origin, with: nil) else {
