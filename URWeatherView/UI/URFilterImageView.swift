...
extension URFilterAppliable where Self: URFilterImageView {
    ...
    public func setFilteredImage(curvePoints: [CGPoint], pointsForRed: [CGPoint]! = DefaultToneCurveInputs, pointsForGreen: [CGPoint]! = DefaultToneCurveInputs, pointsForBlue: [CGPoint]! = DefaultToneCurveInputs) {
        ...
        if !(pointsForRed == DefaultToneCurveInputs && pointsForGreen == DefaultToneCurveInputs && pointsForBlue == DefaultToneCurveInputs) {
...
extension UIImage {
    ...
    public func applyGradientMask(startPoint: CGPoint, endPoint: CGPoint, locations: [CGFloat]) -> CGImage? {
        ...
        guard let context = CGContext.init(data: nil, width: self.cgImage!.width, height: self.cgImage!.height, bitsPerComponent: 8, bytesPerRow: 0, space: colorSpaceGray, bitmapInfo: CGImageAlphaInfo.none.rawValue) else {
        ...
        guard let gradientCGImage = context.makeImage() else {
        ...
        guard let maskingContext = CGContext.init(data: nil, width: self.cgImage!.width, height: self.cgImage!.height, bitsPerComponent: 8, bytesPerRow: 0, space: colorSpaceRGB, bitmapInfo: CGBitmapInfo.byteOrder32Little.rawValue | CGImageAlphaInfo.premultipliedFirst.rawValue) else {
        ...
        guard let maskedCGImage = maskingContext.makeImage() else {
